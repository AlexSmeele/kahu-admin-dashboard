...
export function CSVPreview({ 
  ...
  // Transform data based on mappings and groups
  const transformedData = previewData.map(row => {
    const transformed: any = {};
    
    // Process column groups first
    ...
    // Process regular mappings (skip grouped columns)
  ...
  // Get all target fields for table headers
  const allFields = [
    ...columnGroups.map(g => ({ name: g.targetField, type: 'json' })),
    ...mappings
      .filter(m => m.targetField && !m.isGrouped)
      .map(m => ({ name: m.targetField, type: m.dataType }))
  ];
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Preview of how your data will be imported (showing {previewData.length} of {data.length} rows)
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {data.length > maxRows && (
          <p className="text-sm text-muted-foreground mt-4 text-center">
            + {data.length - maxRows} more rows will be imported
          </p>
        )}
      </CardContent>
    </Card>
